...
# Updated CSS: make tab backgrounds light and all tab/text inside black,
# plus keep high contrast for inputs and glass code panel unchanged.
st.markdown(
    """
    <style>
    :root{
      ...
      --tab-bg: #f8fbff;       /* light tab background */
      --tab-active-bg: #eaf2ff;/* slightly highlighted active tab */
      --tab-text: #041623;     /* tab text black/dark */
    }
    ...
    /* TAB overrides: make tab buttons light and tab text black */
    /* target tab buttons by role attribute (works across Streamlit versions) */
    ...
    /* ensure tab panels content text is black/dark */
    div[role="tabpanel"] {
      color: var(--text) !important;
      background: transparent !important;
    }

    /* Also catch alternative markup: tabs implemented as list items or divs */
    .stTabs button, .stTabs div[role="tab"] {
      background: var(--tab-bg) !important;
      color: var(--tab-text) !important;
    }
    .stTabs button[aria-selected="true"], .stTabs div[role="tab"][aria-selected="true"] {
      background: var(--tab-active-bg) !important;
      color: var(--tab-text) !important;
    }

    /* Ensure placeholders inside inputs remain dark */
    input::placeholder, textarea::placeholder { color: rgba(4,22,35,0.55) !important; }
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("API and timeline shown here. Tab backgrounds are now light and all tab text is black for good readability.")
