...
# Updated CSS: enforce LIGHT input backgrounds and DARK text everywhere inside widgets and tabs
st.markdown(
    """
    <style>
    :root{
      ...
      --input-border: rgba(4,24,40,0.08);
      --input-radius: 8px;
      --mono: 'Courier New', monospace;
      --tab-bg: #f8fbff;
      --tab-active-bg: #eaf2ff;
    }
    ...
    div[role="tablist"] > button[role="tab"] {
      ...
      color: var(--text) !important;
      border: 1px solid rgba(4,24,40,0.06) !important;
    ...
    div[role="tabpanel"] { color: var(--text) !important; background: transparent !important; }

    /* INPUT / TEXTAREA / SELECT styling overrides to ensure light bg + dark text */
    /* Generic inputs */
    ...
    /* Streamlit specific inputs - several commonly generated structures */
    /* TextInput */
    .stTextInput input, .stTextInput .stTextInput input,
    ...
    /* Date input / time input (if present) */
    ...
    /* Placeholder color */
    ::-webkit-input-placeholder { color: rgba(4,22,35,0.55) !important; }
    ::-moz-placeholder { color: rgba(4,22,35,0.55) !important; }
    ::placeholder { color: rgba(4,22,35,0.55) !important; }

    /* Option colors for selects */
    option { color: var(--text) !important; background: white !important; }

    /* Widget labels, captions and help text */
    label, .stMarkdown, .stMarkdown * , .stText, .stText * , .css-1aumxhk, .css-1v3fvcr, .css-1d391kg {
      color: var(--text) !important;
    }
    ...
    button, .stButton>button { color: var(--text) !important; font-weight:700 !important; background: #fff !important; border: 1px solid var(--input-border) !important; }
...
st.markdown(
    ...
    <div style="display:flex; align-items:center; gap:16px; margin-bottom:8px;">
      ...
      <div style="flex:1">
        <div style="font-size:30px; font-weight:800; color:var(--accent);">Decentralized Beauty Contest Game</div>
        <div style="color:var(--muted); margin-top:4px;">Guess 2/3 of the average — commit secretly, reveal later. SHA-256 commitments.</div>
      </div>
      <div style="text-align:right">
        <div style="font-size:12px; color:var(--muted)">Current UTC</div>
...
with st.sidebar:
    ...
    st.markdown(f"<div style='color:var(--text)'>Commit deadline: <strong>{format_dt(COMMIT_DEADLINE_UTC)}</strong></div>", unsafe_allow_html=True)
    st.markdown(f"<div style='color:var(--text)'>Reveal opens: <strong>{format_dt(REVEAL_OPEN_UTC)}</strong></div>", unsafe_allow_html=True)
    st.write("")
    ...
    st.markdown(
        ...
        <div style="margin-bottom:8px;">
          ...
          <div id="commit-countdown" style="display:inline-block;padding:8px 12px;border-radius:10px;background:linear-gradient(90deg,#ffd54a,#ffb347);color:#041623;font-weight:700;">--:--:--</div>
        </div>
        <div style="margin-bottom:8px;">
          <div style="font-weight:700; color:var(--text)">Reveal countdown</div>
          <div id="reveal-countdown" style="display:inline-block;padding:8px 12px;border-radius:10px;background:linear-gradient(90deg,#ffd54a,#ffb347);color:#041623;font-weight:700;">--:--:--</div>
        </div>
    ...
    st.write("")
    if commit_open:
        st.markdown(f"<div style='display:inline-block;padding:6px 10px;border-radius:999px;background:var(--accent);color:white;font-weight:700'>Commits OPEN</div> <div style='color:var(--muted);display:inline-block;margin-left:8px;'>closes in {time_delta_str(COMMIT_DEADLINE_UTC)}</div>", unsafe_allow_html=True)
    else:
        st.markdown(f"<div style='display:inline-block;padding:6px 10px;border-radius:999px;background:#d63031;color:white;font-weight:700'>Commits CLOSED</div>", unsafe_allow_html=True)

    if reveal_open:
        st.markdown(f"<div style='margin-top:6px;display:inline-block;padding:6px 10px;border-radius:999px;background:#0f9d58;color:white;font-weight:700'>Reveals OPEN</div>", unsafe_allow_html=True)
    st.markdown("---")
    st.caption("API and timeline shown here. Input fields are forced to light background with dark text for readability.")
...
with tab_reveal:
    ...
    if preview_hash_cb:
        ...
        if r_uni_v and r_nonce_v is not None:
            ...
            st.markdown("<div style='background:#f0f8ff;padding:8px;border-radius:6px;color:var(--text)'>Computed hash from these reveal inputs:</div>", unsafe_allow_html=True)
